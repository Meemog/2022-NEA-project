...
class Renderer:
    ...
    #Renders a background and a textbox
    ...
    def RenderWaitingText(self, window, screenDimensions):
        textRender = self.__font.render("Waiting for Server", True, (255,255,255))
        renderSize = self.__font.size(str("Waiting for Server"))
        window.blit(textRender, ((screenDimensions[0] - renderSize[0]) / 2, (screenDimensions[1] - renderSize[1]) / 2))